# Provisioning Razor component library

`src/Aetheric.Provisioning.Components` is a Razor class library with the working setup pages, bootstrap backend, optional authentication registration, endpoint mappings, and static web assets. It has no executable entry point or HTML document. `samples/Aetheric.Provisioning.Web` owns those host responsibilities and can be removed without removing the library. Docker currently runs this sample for standalone regression testing.

## Host integration

Reference `Aetheric.Provisioning.Components.csproj`. Register Razor components with Interactive Server support. Register `AddProvisioningBootstrap(configuration, connectionConfiguration)` for the existing local bootstrap backend. This call does not change authentication defaults or map endpoints.
...
This extraction assumes the runtime will supply `IPostSubscriber`. It does not declare a competing interface, implement a subscriber, or invent provisioning message contracts. The preserved bootstrap backend is local to the web host. Runtime provisioning commands/results will be wired through the admin host's messaging adapter once those contracts are available; the browser does not connect directly to RabbitMQ. Moving execution into the runtime and importing the library into admin are separate integration steps.

## Verification

The real HTTP/OIDC tests now discover their setup pages from the library assembly. They exercise account creation/selection, administrator verification/resumption, credential testing and saving, and bootstrap completion through a consuming host. Run `dotnet test --configuration Release`. Run the sample with `dotnet run --project samples/Aetheric.Provisioning.Web`.
